...
def main(request, year=None):
    ...
    return render_to_response('calendar/calendar_page.html', dict(years=lst, user=request.user, year=year),
...
def month(request, year, month, change=None):
    ...
    for day in month_days:
        ...
        if len(lst[week]) == 7:
            lst.append([])
            week += 1
    return render_to_response('calendar/month.html', dict(year=year, month=month, user=request.user,month_days=lst, mname=mnames[month-1]),
                              context_instance=RequestContext(request))
@login_required
